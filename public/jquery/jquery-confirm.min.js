...
options={content:options,title:(option2)?option2:!1};$(this).each(function(){var $this=$(this);$this.on('click',function(e){e.preventDefault();var jcOption=$.extend({},options);if($this.attr('data-title'))
...
options={content:options,title:(option2)?option2:!1};return jconfirm(options)};$.alert=function(options,option2){if(typeof options==='undefined')options={};if(typeof options==='string')
options={content:options,title:(option2)?option2:!1};options.cancelButton=!1;return jconfirm(options)};$.dialog=function(options,option2){if(typeof options==='undefined')options={};if(typeof options==='string')
options={content:options,title:(option2)?option2:!1};options.cancelButton=!1;options.confirmButton=!1;options.confirmKeys=[13];return jconfirm(options)};jconfirm=function(options){if(typeof options==='undefined')options={};if(jconfirm.defaults){$.extend(jconfirm.pluginDefaults,jconfirm.defaults)}
...
if(this.closeIcon===!0){this.$closeButton=this.$b.find('.closeIcon').show()}},setTitle:function(string){this.title=(typeof string!=='undefined')?string:this.title;this.$title.html(this.title||'')},setIcon:function(iconClass){this.title=(typeof string!=='undefined')?iconClass:this.title;this.$icon.html(this.icon?'<i class="'+this.icon+'"></i>':'')},setContent:function(string){var that=this;this.content=(typeof string=='undefined')?this.content:string;if(this.content==''){this.$content.html(this.content);this.$contentPane.hide()}else{this.$content.html(this.content);this.$contentPane.show()}
if(this.$content.hasClass('loading')){this.$content.removeClass('loading');this.$btnc.find('button').prop('disabled',!1)}},_getContent:function(string){var that=this;string=(string)?string:this.content;this._isAjax=!1;if(!this.content){this.content='';this.setContent(this.content);this._contentReady.reject()}else if(typeof this.content==='string'){if(this.content.substr(0,4).toLowerCase()==='url:'){this._isAjax=!0;this.$content.addClass('loading');this.$btnc.find('button').prop('disabled',!0);var url=this.content.substring(4,this.content.length);$.get(url).done(function(html){that.content=html;that._contentReady.resolve()}).always(function(data,status,xhr){if(typeof that.contentLoaded==='function')
that.contentLoaded(data,status,xhr)})}else{this.setContent(this.content);this._contentReady.reject()}}else if(typeof this.content==='function'){this.$content.addClass('loading');this.$btnc.find('button').attr('disabled','disabled');var promise=this.content(this);if(typeof promise!=='object'){console.error('The content function must return jquery promise.')}else if(typeof promise.always!=='function'){console.error('The object returned is not a jquery promise.')}else{this._isAjax=!0;promise.always(function(data,status){that._contentReady.resolve()})}}else{console.error('Invalid option for property content, passed: '+typeof this.content)}
this.setDialogCenter()},_stopCountDown:function(){clearInterval(this.timerInterval);if(this.$cd)
this.$cd.remove()},_startCountDown:function(){var opt=this.autoClose.split('|');if(/cancel/.test(opt[0])&&this.type==='alert'){return!1}else if(/confirm|cancel/.test(opt[0])){this.$cd=$('<span class="countdown">').appendTo(this['$'+opt[0]+'Button']);var that=this;that.$cd.parent().click();var time=opt[1]/1000;this.timerInterval=setInterval(function(){that.$cd.html(' ('+(time-=1)+')');if(time===0){that.$cd.html('');that.$cd.parent().trigger('click');clearInterval(that.timerInterval)}},1000)}else{console.error('Invalid option '+opt[0]+', must be confirm/cancel')}},reactOnKey:function key(e){var a=$('.jconfirm');if(a.eq(a.length-1)[0]!==this.$el[0])
return!1;var key=e.which;if(this.contentDiv.find(':input').is(':focus')&&/13|32/.test(key))
return!1;if($.inArray(key,this.cancelKeys)!==-1){if(this.$cancelButton){this.$cancelButton.click()}else{this.close()}}
...
return!1;if(typeof this.onClose==='function')
this.onClose();this._unwatchContent();that._lastFocused.focus();$(window).unbind('resize.'+this._rand);if(this.keyboardEnabled)
$(window).unbind('keyup.'+this._rand);that.$el.find('.jconfirm-bg').removeClass('seen');$('body').removeClass('jconfirm-noscroll');this.$b.addClass(this.closeAnimation);var closeTimer=(this.closeAnimation=='anim-none')?0:this.animationSpeed;setTimeout(function(){that.$el.remove()},closeTimer*25/100);jconfirm.record.closed+=1;jconfirm.record.currentlyOpen-=1;return!0},open:function(){var that=this;if(this.isClosed())
return!1;that.$el.find('.jconfirm-bg').addClass('seen');this.$b.removeClass(this.animation);this.$b.find('input[autofocus]:visible:first').focus();jconfirm.record.opened+=1;jconfirm.record.currentlyOpen+=1;if(typeof this.onOpen==='function')
this.onOpen();var jcr='jconfirm-box'+this._rand;this.$b.attr('aria-labelledby',jcr).attr('tabindex',-1).focus();if(this.$title)
this.$title.attr('id',jcr);else if(this.$content)
this.$content.attr('id',jcr);setTimeout(function(){that.$b.css({'transition-property':that.$b.css('transition-property')+', margin'});that._modalReady.resolve()},this.animationSpeed);return!0},isClosed:function(){return this.$el.css('display')===''}};jconfirm.pluginDefaults={template:'<div class="jconfirm" align="center"><div class="jconfirm-bg"></div><div class="jconfirm-scrollpane"><div class="container"><div class="jconfirm-box-container"><div class="jconfirm-box" role="dialog" aria-labelledby="labelled" tabindex="-1"><div class="closeIcon">&times;</div><div class="title-c"><span class="icon-c"></span><span class="title"></span></div><div class="content-pane"><div class="content"></div></div><div class="buttons"></div><div class="jquery-clear"></div></div></div></div></div></div>',title:'Hello',content:'Are you sure to continue?',contentLoaded:function(){},icon:'',opacity:0.2,confirmButton:'Okay',cancelButton:'Close',confirmButtonClass:'btn-default',cancelButtonClass:'btn-default',theme:'white',animation:'zoom',closeAnimation:'scale',animationSpeed:500,animationBounce:1.2,keyboardEnabled:!1,rtl:!1,confirmKeys:[13],cancelKeys:[27],container:'body',confirm:function(){},cancel:function(){},backgroundDismiss:!1,autoClose:!1,closeIcon:null,closeIconClass:!1,watchInterval:100,columnClass:'col-sm-6 col-sm-offset-3',onOpen:function(){},onClose:function(){},onAction:function(){}};jconfirm.record={opened:0,closed:0,currentlyOpen:0}})(jQuery)